...
image_base64 = encode_image_to_base64("aa1.jpg")
if image_base64:
    st.markdown(f"""
    <style>
...
def load_trophy_data():
    df = pd.read_csv("all_champions_trophy_matches_results.csv", dtype=str)
    df.columns = df.columns.map(str)
...
""", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(
            ...
            "📊 **Player Analysis**<br>Get complete stats of your favorite players."
            "</div>", unsafe_allow_html=True)

    with col2:
        st.markdown(
            "<div style='background-color:#FFF5EE; padding:15px; border-radius:10px; text-align:center;'>"
            "📈 **Player Graphs**<br>Visualize runs, averages, and more over the years."
            "</div>", unsafe_allow_html=True)

    with col3:
        st.markdown(
            "<div style='background-color:#F5FFFA; padding:15px; border-radius:10px; text-align:center;'>"
            "👤 **Player comparison**<br>Comparison between two player"
            "</div>", unsafe_allow_html=True)
